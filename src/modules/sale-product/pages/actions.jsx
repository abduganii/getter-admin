    ...
    fields={[
      ...
      },
      {
        ...
        customData={(data) => {
          ...
          retrData.developers = developer?.items.filter(item => retrData?.developers?.includes(item.id));
          retrData.categories = categories?.items.find(item => retrData?.categories?.includes(item.id));
          return retrData
        }}
    onSuccess={() => {
            ...
            <div className="w-full max-w-[947px] mx-auto gap-[31px] mb-10  overflow-hidden flex ">
              ...
              <GlobalIcons
                ...
                <GlobalIcons
                  ...
                  onDeselect={(e) => {
                    formik.setFieldValue(`directions`, formik.values.directions.filter(el=>el.title !=  ('#'+e)));
                  }}
                    onSelect={async (e) => {
                    if (e) {
                    await AddData('tag', {
                      ...
                      .then((res) => {
                      formik.setFieldValue(`directions`, [...formik.values.directions,res.data ]);
                    })
              ...
              <GlobalIcons
                    ...
                    required={true}
                />
                <FileUpload name={'avatar'} formik={formik} value={formik.values.avatar} title={""} />
                <GlobalIcons
                    placeholder={"link"}
                    type="text"
                    className="w-full"
                ...
                />
                 <GlobalIcons
                  ...
                  onDeselect={(e) => {
                    formik.setFieldValue(`technologies`, formik.values.technologies.filter(el=>el.title !=  ('#'+e)));
                  }}
                    onSelect={async (e) => {
                    if (e) {
                    await AddData('tag', {
                      ...
                      .then((res) => {
                    formik.setFieldValue(`technologies`, [...formik.values.technologies,res.data ]);
                    })
                    .catch(()=>console.log("err"))
                    }
                  }}
